...
while parameter_values(end) >= 0.5 %Question: Maybe we can accomodate some sort of margin, or do without this condition?
    ...
    if theta >= 0
        ...
        f = @(x) -((2*alpha_t_1 + theta/x)/2) + (d*(x - 1))^2;
        options = optimoptions('fminunc','Display','off');
        g_theta = fminunc(f,1,options);
        if g_theta <= 0
            g_theta = 0;
        end
    end
    ...
    fprintf('d = %f, alpha_t_1 = %f, time_period = %d, theta = %f, g_theta = %f, s = %f, alpha_t = %f\n', parameter_values);
end

% Plot alpha_t over time
figure;
plot(1:time_period, parameter_values(7:end), '-o');
xlabel('Time Period');
ylabel('Alpha_t');
title('Government Affinity over Time');

% Save plot as PNG file
saveas(gcf, 'affinity_plot.png');
